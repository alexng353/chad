...
export const RUN_FLAGS: FlagDef[] = [
	...
	},
	{
		short: "-b",
		long: "--box-height",
		description: "Box height in lines (default: 30)",
		takesValue: true,
	},
	{ long: "--dry-run", description: "Show the next step without running" },
	{ long: "--model", description: "Model to use", takesValue: true },
];

const DEBUG_LOG = "/tmp/chad-debug.log";

export function runHandler(ctx: RouteContext): Promise<void> {
...
export async function executeRun(
	...
	const { flags, config, chadDir } = ctx;
		...
		switch (msg.type) {
			case "system":
				if (msg.subtype === "init") {
					...
					if (msg.model) {
						box.addLine(
							`${ansi.dim("model:")} ${ansi.cyan(String(msg.model))}`,
						);
					}
					draw();
				}
